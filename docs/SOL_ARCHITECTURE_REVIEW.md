...
| Medium   | Counterfactual reversal  | A binary sign check can call a small retracement a full reversal and hide sparse-horizon observation delay.                                                                                     | Evaluation now computes retained-movement ratio at 30/60/300 seconds, requires both source progression and received availability inside the maximum-lag window, records received-time observations, and compares settled immediate/confirmation unit returns. Default persisted/inconclusive/reversed thresholds are 0.60 and 0.00. | All three horizons, delayed/expired-horizon rejection, received-time lag, stable and small-retracement persistence, inconclusive partial retention, full reversal, and entry comparison. |
...
| Medium   | OpenAPI                  | Route summaries without request/response models are insufficient for judge discovery and client integration.                                                                                    | Reusable Fastify/OpenAPI schemas now describe complete nested domain responses, enums, formats, control requests, parameters, and common errors.                                                                                                                                                                                    | Structural schema/property/reference assertions, `/docs/json`, endpoint status/shape checks, invalid control, and audit-limit cases.                                                     |
| Medium   | CLI/live readiness       | A configuration check can be mistaken for authenticated stream health, and one-shot replay commands can be mistaken for persistent controls.                                                    | `txline check` is documented as configuration-only; the running process exposes session-free `/api/live/status`, while replay readiness remains independent. CLI documentation labels one-shot and ephemeral behavior.                                                                                                              | Disabled/configured live status, credential redaction, session-free route, and CLI help/command smoke checks.                                                                            |
...
| Medium   | Runtime retention        | Live records, stream identity caches, and process audit arrays can grow without bounds; full-history cloning amplifies memory use.                                                              | Official live record/message/ID/sequence retention is capped, response/event sizes are bounded, sessions/TTL are bounded, audit refuses append at 2,000 events, and replay actions reserve audit capacity before mutating state.                                                                                                    | Live cache eviction, oversized input, ordering, and fail-closed audit-capacity regressions.                                                                                              |
| Low      | Secret/build hygiene     | Git ignores alone do not prevent credentials, wallet files, or generated audit exports from entering a Docker build context.                                                                    | Secret, wallet, credential, audit-export, report, and generated-output patterns are mirrored in ignore policy; signing is confined to an explicit one-shot devnet tool and the server runtime receives no wallet.                                                                                                                   | Repository/history/context secret scan, credential redaction tests, and external mode-`0600` wallet/env checks.                                                                          |
...
The simulation, replay, quality, audit, and lifecycle hardening established broad regression coverage. The later live-devnet tranche adds official-shape adapters, malformed payloads, SSE fragmentation, heartbeat/idle timeout, retry/backoff, cancellation, JWT renewal, redaction, ordering, proof classification, mocked HTTP/SSE integration, and LIVE/REPLAY UI coverage. Exact final counts belong to the post-merge release log.
...
- multiple eligible event correlation with directionally supported candidate selection;
...
- retained-movement classifications and dual source/availability timing at 30, 60, and 300 seconds.
...
- repeated/conflicting terminal events are rejected without mutating settled fixture state;
- a flat market response does not satisfy event-confirmed movement;
- audit cloning, run IDs, global sequence, and completeness.

### Interfaces

- required API routes and complete nested OpenAPI schemas;
- replay validation/error responses, session-free readiness, bounded session capacity, scheduler failure containment, and simulation disclaimers;
- judge dashboard flow from reset through signal, operational warning, recovery, settlement, analytics, score components, signal detail, and audit;
...
The final release checkpoint must record formatter, lint, strict TypeScript, unit/integration, production build, Chromium, dependency audit, Docker, real credentialed devnet smoke, GitHub Actions, and public Render results after the integrated branch is complete.

## Remaining limitations

- The bundled match is synthetic, not recorded TxLINE data.
- The live sidecar is read-only and does not drive paper decisions. Official integer odds remain unconverted until TxLINE supplies the applicable decimal scale.
...
- Counterfactual points use the first snapshot whose source progression and received availability both fit the horizon window, do not interpolate, and remain absent when the configured 30-second observation-lag limit is exceeded.
...
- A durable audit store, pagination/retention, long-running live observability, and multi-replica coordination remain future production work.
- Replay is public at <https://txline-sentinel.onrender.com>; the latest live sidecar is not public until Render secrets, exact-commit deployment, and `/api/live/status` are verified.

## Conclusion

The project now combines explicit mathematical, causal, simulation, and audit semantics with an authenticated official TxLINE devnet transport and real on-chain fixture-proof simulation. Replay remains publicly available and deterministic. The remaining limits are documented product boundaries; public live status must not be claimed before the exact live commit and Render environment are verified.
